...
1. [ ] Add `NeighborVideo` Pydantic model and update `Source` model in `backend/main.py`.
...
2. [ ] Implement neighbor lookup in `backend/answer.py`.
...
3. [ ] Update `frontend/app/page.tsx` — interfaces.
...
4. [ ] Render the "Also in this series" strip in `frontend/app/page.tsx`.
